...
def main():
    ...
    #
    secret_pass_len = 8
    suf_len = len(url.suffix)
    pad = padding(secret_pass_len + suf_len)

    hash1 = sha256(
        state=bytes.fromhex(url.token),
        count=secret_pass_len + suf_len + len(pad),
    )

    suf_unlock_safes = '&command=UnlockSafes'.encode()
    hash1.update(suf_unlock_safes)

    url.token = hash1.hexdigest()
    url.suffix += quote(pad) + '&command=UnlockSafes'
