 ...
 *     grtfs_size(), a return value > MAX_FILE_SIZE is used to
 *     indicate an error)
 *
...
unsigned int grtfs_seek(   unsigned int file_descriptor,
                         unsigned int offset );
...
unsigned int file_is_writeable( char* name );
unsigned int make_readable( char* name );
unsigned int make_writable( char* name );
