...
bool isSafe(bitset<4> position){
  // Check if the wolf and goat are together without the person 
  // or the goat and the cabbage without the person
  ...
  while(curState != last){
    for(int i = 0; i < 4; i++){
      if(curState[me] == curState[i]){
        ...
        if(isSafe(next)){
          int current = (int)curState.to_ulong();
          int nextNum = (int)next.to_ulong();
          nbrs[current].push_back(nextNum);
          edge_label[make_pair(current,nextNum)] = neighbor_label(current, nextNum);
        }
      }
    }
