...
bool refine(double &len)
{
  ...
  for(auto& city : cities){
    double edgeDist = dist(city.second, city.second + 1);
    for(int i = city.second + 2; i <= cities.size(); ++i){
      double tempDist = dist(i, i + 1);
      double edgeTemp = edgeDist + tempDist;
      double newEdges = dist(city.second, i) + dist(city.second + 1, i + 1);

      //New distance is shorter so flip them
      if(edgeTemp > newEdges && edgeTemp - newEdges > .0001){
        //Swap the diagnals
        swap(P[city.second], P[city.second + 1]);
        len = len - edgeTemp + newEdges;
      }
    }
  }
}
