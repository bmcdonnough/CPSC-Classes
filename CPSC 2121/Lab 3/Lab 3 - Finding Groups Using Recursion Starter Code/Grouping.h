...
class Grouping
{
    private:
        ...
        bool valFinder(GridSquare key); //Checks for repeat values in groups
    public:
        Grouping(); //Default constructor, no groups
        Grouping(std::string fileName); //Implement this function
        void findGroup(int r, int c, vector<GridSquare> &currentGroup, int recCtr); //Implement this function (recursive)
