...
class FileTransferServer:
    ...
    def start(self):
        ...
        try:
            ...
            self.logger.info("Server binded to %s" %(self.port))
            ...
            self.logger.info("Error Starting Server")
            return False
    ...
    def run(self):
        ...
        try:
            while (self.is_running):
                ...
                connected = True
                while connected:
                    metadata = client.recv(self.BUFFER_SIZE)
                    ...
                    filename, file_extension, created, file_size, hash_length, remaining_data = self.unpack_metadata(metadata)
                    
                    received_data = remaining_data
                    while len(received_data) < file_size:
                        remaining_size = file_size - len(received_data)
                        amount_left = min(remaining_size, self.BUFFER_SIZE)
                        amount_left = client.recv(amount_left)
                        received_data += amount_left
                        metadata += amount_left
                        
                    client_hash = client.recv(hash_length)
                        
                    server_hash = self.compute_hash(metadata, hash_length)
                    
                    if (server_hash == client_hash):
                        self.write_file(filename, received_data)
                        self.logger.info(f"File '{filename}' received and saved successfully.")
                    else:
                        self.logger.info(f"Error: Hash mismatch for file '{filename}'.")
                        
                    client.send(server_hash)
                    
        except TimeoutError:
            self.logger.info("Error: Timed Out")
        except OSError:
            self.logger.info("OS Error")
        except Exception as e:
            self.logger.info(f"Unexpected Error {e} Occurred")
    ...
    def unpack_metadata(self, data):
        ...
        """
        # TODO: Implement the functionality described in this function's docstring
        filename_length, file_extension_length, created_length = unpack(f"!HBB", data[:4])
        filename = data[4:(4 + filename_length)].decode()
        file_extension = data[(4 + filename_length):(4 + filename_length + file_extension_length)].decode()
        buffer = 4 + filename_length + file_extension_length
        created = data[buffer:(buffer + created_length)].decode()
        file_size, hash_length = unpack(f"!IB", data[buffer + created_length: buffer + created_length + 5])
        buffer += created_length + 5

        file_data = data[buffer: buffer + file_size]
        hash_value = data[buffer+file_size: buffer+file_size+hash_length]  

        remaining_data = file_data + hash_value

        data_tuple = (filename, file_extension, created, file_size, hash_length, remaining_data)
        return data_tuple
    ...
    def compute_hash(self, data, hash_length):
        ...
        shake_hash.update(data)
        return shake_hash.digest(hash_length)
...
if __name__ == "__main__":
    server = FileTransferServer(1234)
    if server.start():
        server.run()
