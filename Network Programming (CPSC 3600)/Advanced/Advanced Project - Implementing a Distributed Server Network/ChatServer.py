...
##############################################################################################################
    ...
    def setup_server_socket(self):
        ...
        # Set socket options (optional but recommended for graceful re-binding)
        ...
        server_socket.listen(5)  # Argument defines the max backlog queue length

        # Register the socket with the selector to handle read events (incoming connections)
        self.sel.register(server_socket, selectors.EVENT_READ, data='server_socket')

        # Store the server socket as an instance attribute
    ...
    def connect_to_server(self):
        ...
        # Step 2: Connect the client socket to the specified remote server
        try:
            client_socket.connect( (self.connect_to_host_addr, self.connect_to_port) )
            #self.print_info(f"Connected to another server")
        except Exception as e:
            self.print_info(f"Failed to connect to server: {e}")
            return
        
        # Step 3: Register the client socket with the selector to handle read events (incoming messages)
        connection_data = BaseConnectionData()
        self.sel.register(client_socket, selectors.EVENT_READ, data=connection_data)

        # Step 4: Send a ServerRegistrationMessage to the connected server
        registration_message = ServerRegistrationMessage.bytes(
            source_id=self.id,
            last_hop_id=0,
            server_name=self.server_name,
            server_info=self.server_info
        )
        client_socket.send(registration_message)
        ...
        try:
            while not self.request_terminate:
                ...
                for key, event_mask in events:
                    io_device = key.fileobj
                    if io_device == self.server_socket:
                        if event_mask & selectors.EVENT_READ:
                            self.accept_new_connection(io_device)
                    else:
                        self.handle_io_device_events(io_device, event_mask)
        except Exception as e:
            self.print_info(f"An error occurred in the event loop: {e}")
    ...
    def cleanup(self):
        ...
        # TODO: Implement the above functionality
        if self.sel:
            registered_sockets = list(self.sel._fd_to_key.values())
        
        for key in registered_sockets:
            io_device = key.fileobj
            try:
                if io_device.fileno() != -1:
                    self.sel.unregister(io_device)
                    io_device.close()
            except Exception as e:
                self.print_info(f"Error while closing socket {io_device}: {e}")
        ...
        if hasattr(self, 'server_socket') and self.server_socket:
            try:
                ...
                self.print_info(f"Error while closing server socket: {e}")
            
        try:
            self.sel.close()
        except Exception as e:
            self.print_info(f"Error while closing the selector: {e}")
    ...
    def accept_new_connection(self, io_device):
        ...
        # TODO: Implement the above functionality
        try:
            new_connection, addr = io_device.accept()
            new_connection.setblocking(False)
            connection_data = BaseConnectionData()
            self.sel.register(new_connection, selectors.EVENT_READ | selectors.EVENT_WRITE, connection_data)
            #self.print_info(f"Accepted a connection from another server")
        
        except BlockingIOError:
            self.print_info("No new connection available to accept.")
        
        except Exception as e:
            self.print_info("Error while accepting new connection: {}".format(str(e)))
            if new_connection:
    ...
    def handle_io_device_events(self, io_device, event_mask):
        ...
        if event_mask & selectors.EVENT_READ:
            try:
                ...
                self.sel.unregister(io_device)
                io_device.close()
                self.print_info("Error reading from socket: " + str(e))
        
        if event_mask & selectors.EVENT_WRITE:
            connection_data = self.sel.get_key(io_device).data
            if connection_data.write_buffer:
                try:
                    io_device.send(connection_data.write_buffer)
                    connection_data.write_buffer = b''
                except OSError as e:
                    self.sel.unregister(io_device)
                    io_device.close()
                    self.print_info("Error writing to socket: " + str(e))
...
##############################################################################################################
    ...
    def send_message_to_host(self, destination_id, message):
        ...
        if destination_id in self.hosts_db:
            connection_data = self.hosts_db[destination_id]
            self.print_info("Sending message to Host ID #%s \"%s\"" % (destination_id, message))
            connection_data.write_buffer += message
        else:
            self.print_info("Destination ID #%s not found in hosts_db" % destination_id)
    ...
    def broadcast_message_to_servers(self, message, ignore_host_id=None):
        ...
        """
        # TODO: Implement the above functionality
        for i in range(len(self.adjacent_server_ids)):
            if ignore_host_id is None or self.adjacent_server_ids[i] != ignore_host_id:
                self.send_message_to_host(self.adjacent_server_ids[i], message)
    ...
    def broadcast_message_to_adjacent_clients(self, message, ignore_host_id=None):
        ...
        """
        # TODO: Implement the above functionality
        for i in range(len(self.adjacent_user_ids)):
            if ignore_host_id is None or self.adjacent_user_ids[i] != ignore_host_id:
                self.send_message_to_host(self.adjacent_user_ids[i], message)
...
##############################################################################################################
    ...
    def handle_server_registration_message(self, io_device, message):
        ...
        clients. You can accomplish this by creating ServerRegistrationMessages and ClientRegistrationMessages 
        ...
        # Server is already registered in database
        if server_id in self.hosts_db:
            status_message = StatusUpdateMessage.bytes(
                source_id=self.id,
                destination_id=0,
                message_code=0x02,
                content=f"A machine has already registered with ID {server_id}"
            )
            self.send_message_to_unknown_io_device(io_device, status_message)
            return
                
        new_server = ServerConnectionData(
            id=server_id,
            server_name=message.server_name,
            server_info=message.server_info,
        )
        
        # Register adjacent server
        if message.last_hop_id == 0 or message.last_hop_id == server_id:
            new_server.first_link_id = server_id
            self.adjacent_server_ids.append(server_id)
            self.sel.modify(
                io_device,
                selectors.EVENT_READ | selectors.EVENT_WRITE,
                new_server,
            )
            self.hosts_db[server_id] = new_server

            # Server is new to the network
            if message.last_hop_id == 0:
                self_message = ServerRegistrationMessage.bytes(
                    source_id=self.id,
                    last_hop_id=self.id,
                    server_name=self.server_name,
                    server_info=self.server_info,
                )
                self.send_message_to_host(server_id, self_message)
                
                # Send new server info about the entire network
                for host_id, host in self.hosts_db.items():
                    if host.id is not server_id:
                        if isinstance(host, ServerConnectionData):
                            registration_message = ServerRegistrationMessage.bytes(
                                source_id=host.id,
                                last_hop_id=self.id,
                                server_name=host.server_name,
                                server_info=host.server_info,
                            )
                            self.send_message_to_host(server_id, registration_message)
                            
                        if isinstance(host, ClientConnectionData):
                            registration_message = ClientRegistrationMessage.bytes(
                                source_id=host.id,
                                last_hop_id=self.id,
                                client_name=host.client_name,
                                client_info=host.client_info,
                            )
                            self.send_message_to_host(server_id, registration_message)
                
                # Broadcast message to all other servers about new server
                broadcast_message = ServerRegistrationMessage.bytes(
                    source_id=server_id,
                    last_hop_id=self.id,
                    server_name=message.server_name,
                    server_info=message.server_info,
                )
                self.broadcast_message_to_servers(broadcast_message, ignore_host_id=server_id)
        
        # Register non-adjacent server
        else:
            new_server.first_link_id = message.last_hop_id
            self.hosts_db[server_id] = new_server
            broadcast_message = ServerRegistrationMessage.bytes(
                    source_id=server_id,
                    last_hop_id=self.id,
            ...
            # Relay server info to rest of network
            self.broadcast_message_to_servers(broadcast_message, ignore_host_id=message.last_hop_id)
...
##############################################################################################################
    ...
    def handle_client_registration_message(self, io_device, message):
        ...
        if message.source_id in self.hosts_db:
            self.send_message_to_unknown_io_device(io_device, StatusUpdateMessage.bytes(source_id=self.id,destination_id=0,message_code=0x02,content=f"Someone has already registered with ID {message.source_id}"))
            return
        n_client_connection_data = ClientConnectionData(id=message.source_id,client_name=message.client_name,client_info=message.client_info)
        if message.last_hop_id == 0 or message.last_hop_id == message.source_id:
            n_client_connection_data.first_link_id = message.source_id
            self.adjacent_user_ids.append(message.source_id)
            self.sel.modify(io_device, selectors.EVENT_READ | selectors.EVENT_WRITE, n_client_connection_data)
            self.hosts_db[message.source_id] = n_client_connection_data
            if message.last_hop_id == 0:
                self.send_message_to_host(message.source_id, StatusUpdateMessage.bytes(source_id=self.id, destination_id=message.source_id, message_code=0x00, content="Welcome to the Clemson Relay Chat network %s" % message.client_name))
                for host_id, host in self.hosts_db.items():
                    if host.id is not message.source_id:
                        if isinstance(host, ClientConnectionData):
                            self.send_message_to_host(message.source_id, ClientRegistrationMessage.bytes(source_id=host.id, last_hop_id=self.id, client_name=host.client_name, client_info=host.client_info))
                self.broadcast_message_to_servers(ClientRegistrationMessage.bytes(source_id=message.source_id, last_hop_id=self.id, client_name=message.client_name, client_info=message.client_info), ignore_host_id=message.source_id)
                self.broadcast_message_to_adjacent_clients(ClientRegistrationMessage.bytes(source_id=message.source_id, last_hop_id=self.id, client_name=message.client_name, client_info=message.client_info), ignore_host_id=message.source_id)
        else:
            n_client_connection_data.first_link_id = message.last_hop_id
            self.hosts_db[message.source_id] = n_client_connection_data
            self.broadcast_message_to_servers(ClientRegistrationMessage.bytes(source_id=message.source_id, last_hop_id=self.id, client_name=message.client_name, client_info=message.client_info), ignore_host_id=message.last_hop_id)
            self.broadcast_message_to_adjacent_clients(ClientRegistrationMessage.bytes(source_id=message.source_id, last_hop_id=self.id, client_name=message.client_name, client_info=message.client_info), ignore_host_id=message.last_hop_id)
...
##############################################################################################################
    ...
    def handle_status_message(self, io_device, message):
        ...
        if destination_id == self.id or destination_id == 0:
            self.status_updates_log.append(message.content)
            
        elif destination_id in self.hosts_db:
            if destination_id in self.adjacent_user_ids or self.adjacent_server_ids:
                self.send_message_to_host(destination_id, message.content)
            else:
                self.send_message_to_host(destination_id, message.content)
        else:
            self.print_info("Warning: Status message intended for unknwon ID %s" % message.destination_id)
...
##############################################################################################################
    ...
    def handle_client_chat_message(self, io_device, message):
        ...
        if destination_id in self.hosts_db:
            if destination_id in self.adjacent_user_ids:
                self.send_message_to_host(destination_id, message.bytes)
            else:
                self.send_message_to_host(self.hosts_db[destination_id].first_link_id, message.bytes)
        else:
            unknown_id_message = f"Unknown ID {destination_id}"
            status_update = StatusUpdateMessage.bytes(
                source_id=self.id,
                destination_id=message.source_id,
                message_code=0x01,
                content=unknown_id_message
            )
            self.send_message_to_host(message.source_id, status_update)
...
##############################################################################################################
    ...
    def handle_client_quit_message(self, io_device, message):
        ...
        if client_id in self.hosts_db:
            self.broadcast_message_to_adjacent_clients(message.bytes, ignore_host_id=client_id)
            self.broadcast_message_to_servers(message.bytes, ignore_host_id=client_id)
            del self.hosts_db[client_id]
            if client_id in self.adjacent_user_ids:
                self.adjacent_user_ids.remove(client_id)
...
##############################################################################################################    
    ...
    # You are encouraged to use print_info, print_debug, and print_info to log
