...
import os, time, sys, logging, hashlib
...
class FileTransferClient:
    ...
    def send_file(self, filepath):
        ...
        # TODO: Implement the functionality described in this function's docstring
        try:
            encoded_data = self.encode_metadata(filepath)
            
            file_data = self.read_file(filepath)
            full_data = encoded_data + file_data
            hash_value = self.compute_hash(full_data)
            
            full_data_with_hash = full_data + hash_value
            
            self.client_socket.send(full_data_with_hash)
            
            server_hash = self.client_socket.recv(self.BUFFER_SIZE)
            print("yay i got it")
            ...
            self.logger.info("Error: Connection Reset")
        except ConnectionAbortedError:
            self.logger.info("Error: Connection Aborted")
        except OSError:
            self.logger.info("Unexpected Error Occurred")
    ...
    def encode_metadata(self, filepath):
        ...
        format_string = f"!HBB{len(filename)}s{len(file_extension)}s{len(created_date)}sIB"
    ...
    def compute_hash(self, data):
        ...
        return shake_hash.digest(self.hash_length)
...
if __name__ == "__main__":
    client = FileTransferClient("localhost", 1234)
    if client.start():
        client.send_file("source/test1.jpg")
