...
class TCPServer:
        ...
        while self.is_running:
            ...
            while connected:
                ...
                if message != "quit":
                    ...
                    #If the client says quit, close the connection
                    connected = False
                    client.close()
    ...
    def process_request(self, data):
        ...
        #Make sure that there are 3 parameters
        if len(operands) != 3:
            raise ValueError("ERROR: Invalid number of parameters. Valid format is: <OPERATION> <OPERAND1> <OPERAND2>")
        
        try:
            int(operands[1]) == operands[1] and int(operands[2]) == operands[2]
        except ValueError:
            raise ValueError("ERROR: Operands must be valid integers")
        
        #Handle all cases from the client
        if operands[0] == "ADD":
            #Read in the numbers and add them together to send back to the client
            num1 = operands[1]
            num2 = operands[2]
            result = int(num1) + int(num2)

        elif operands[0] == "SUB":
            #Read in the numbers, subtract them to send back to the client 
            num1 = operands[1]
            num2 = operands[2]
            result = int(num1) - int(num2)

        elif operands[0] == "MUL":
            #Read in the numbers, multiply them, to send back to the client
            num1 = operands[1]
            num2 = operands[2]
            result = int(num1) * int(num2)
            ...
            #Read in the numbers, divide them, to send back to the client
            num1 = operands[1]
            num2 = operands[2]
            print(num2)
            if int(num2) == 0:
                raise ValueError("ERROR: Division by zero error. The second operand cannot be zero.")
            else:
                result = int(num1) / int(num2)

        else:
            raise ValueError('ERROR: Invalid operation. Valid operations are ADD, SUB, DIV, and MUL')
